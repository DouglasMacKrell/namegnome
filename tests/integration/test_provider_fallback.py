...
class TestProviderFallback:
    ...
    def test_tvdb_failure_tmdb_success(self, tmp_path: Path) -> None:
        ...
        with (
            patch("namegnome.metadata.episode_fetcher._provider_tvdb") as mock_tvdb,
    ...
    def test_tvdb_tmdb_omdb_failure_anilist_success(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("namegnome.metadata.episode_fetcher._provider_omdb") as mock_omdb,
            patch(
                "namegnome.metadata.episode_fetcher._provider_anilist"
            ) as mock_anilist,
        ):
            mock_tvdb.side_effect = Exception("TVDB failed")
            mock_tmdb.side_effect = Exception("TMDB failed")
    ...
    def test_provider_unhealthy_marking(self) -> None:
            ...
            return [
                {
                    "season": season or 1,
                    "episode": 1,
                    "title": f"TMDB {show} Episode 1",
                },
                {
                    "season": season or 1,
                    "episode": 2,
                    "title": f"TMDB {show} Episode 2",
                },
            ]
        ...
        with (
            patch("namegnome.metadata.episode_fetcher._provider_tvdb", failing_tvdb),
            ...
            try:
                ...
                assert episodes[0]["title"].startswith("TMDB"), (
                    "Should use TMDB fallback"
                )
            except Exception:
