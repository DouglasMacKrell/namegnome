...
* **LLM Agent** – delegates fuzzy matching and edge-case reasoning; default model *llama3:8b* via Ollama (recommended). Lightweight fallback *mistral:7b* and code-focused fallback *deepseek-coder-v2:16b-lite-instruct* are also supported and selectable via `--llm-model` or `namegnome llm set-default`.
...
| **Testing**             | Pytest                                                                              | Sole test runner; coverage 80 %                                                                              |
...
| **LLM**                 | Ollama local server (default `llama3:8b`)                                           | Offline inference, user-switchable; Mistral 7B and DeepSeek-Coder-V2-Lite available as alternates            |
...
4. **Platform presets & extensibility** – first-class flags for
   ...
   * lightweight/DLNA targets: `universal-media-server`, `gerbera`, `minidlna`
     Preset determines naming conventions, tag strategy, and which metadata providers are queried.  New servers can be added via a plug-in folder without touching core code.
   * **Media-type handlers** – TV, Film, Music (strictly isolated per domain).
   * Anime flag enables absolute numbering heuristics.
...
* Best lightweight LLM for local semantic similarity on filenames (<8 GB VRAM)? (`mistral:7b` currently fills this niche.)
