...
* **Steps:**
  ...
  3. **Windows specifics** – Use `os.replace` which is atomic on NTFS; prepend `"\\?\"` to both paths when `len(path) > 259` to bypass MAX\_PATH. Handle ACL inheritance by copying DACLs via `ctypes` if `overwrite`.
...
* **Steps:**
  ...
  1. Create `metadata/cache.py` with SQLite table: provider, key\_hash, json\_blob, expires\_ts.
...
* **Steps:**
  ...
  1. Implement size check: prompt length > 10\_000 chars or > 2 MB aborts with warning.
...
*(Move tasks here with completion date)*
