...
* **Atomic sub-tasks:**
  ...
  | **1.6-k** | **Complete regression coverage** | Add remaining edge case tests to achieve 100% SCAN_RULES.md compliance: (1) Show name disambiguation when multiple series match (e.g., "Danger Mouse" → prompt for 1989 vs 2015 version), (2) Absolute path requirement enforcement, (3) Duration-based edge cases (double-length episodes, mismatched durations), (4) Provider timeout/retry behavior validation. Focus on show disambiguation as highest priority since it's user-facing and prone to regression. |
