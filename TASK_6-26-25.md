...
* **Atomic sub-tasks:**
  ...
  | **1.6-m** | **Complete anthology show E2E coverage** | **Goal**: Expand E2E tests to cover all 6 shows from fixture manifest, ensuring anthology edge cases are properly validated. **Current gap**: E2E tests only use Danger Mouse 2015 (non-anthology) and miss complex anthology scenarios that represent majority of our edge cases. **Missing shows**: (1) Firebuds - anthology with trusted episode titles, (2) Harvey Girls Forever! - anthology with untrusted names from Sonarr, (3) Martha Speaks - anthology with important edge cases, (4) Paw Patrol - anthology with complex episode mapping, (5) The Octonauts - special case anthology handling. **Implementation**: Create comprehensive `e2e_anthology_test_files` fixture that samples from all 6 shows, add parametrized tests for each show type, validate anthology-specific logic (episode title parsing, untrusted names handling, complex mapping). **Success criteria**: E2E tests exercise complete anthology pipeline, all 6 show types covered, anthology-specific edge cases validated. |
