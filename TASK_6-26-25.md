...
* **Atomic sub-tasks:**
  ...
  | **1.6-e** | **Provider-fallback matrix** | Create `tests/integration/test_provider_fallback.py` exercising: (1) TVDB ok; (2) TVDB 5xx → TMDB ok; (3) TVDB + TMDB 5xx → AniList ok; (4) all providers fail → items flagged `unsupported`, exit 3. Uses helper `tests/helpers/provider_mocks.py` built on `respx`. |
  | **1.6-f** | **Unsupported / malformed inputs** | Introduce `tests/mocks/tv/unsupported/` containing bad extensions & malformed names; assert they surface as `unsupported`. |
  | **1.6-g** | **Performance guard-rail** | Param test measuring scan runtime (< 5 s with network mocked); fails if exceeded to catch accidental O(n²) behaviour. |
  | **1.6-h** | **CI wiring** | Add fast `integration-tv` job (Linux, `--no-rich`) plus nightly cross-platform matrix; update coverage gate to include integration folder. |
  | **1.6-i** | **Documentation update** | Extend `SCAN_RULES.md` (confidence thresholds, exit-code semantics) and README "Regression Suite" section; document manifest format & how to run locally. |
  | **1.6-j** | **Untrusted-names span logic** | Add Harvey Girls Forever! (and similar) fixtures to manifest with `title_trusted: false`. Write a param test that mocks TVDB runtimes and asserts that Sonarr-renamed files are mapped to correct episode spans using only input numbering and canonical episode durations, ignoring input titles. Ensure output filenames replace '!' with safe punctuation per naming rules. |
...
* **Done when:**
  * All sub-tasks **1.6-a** → **1.6-j** are merged and green in CI.
